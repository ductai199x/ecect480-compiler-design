...
class Expression
{
  ...
  public:
    ...
    bool isExprUndef() { return type == ExpressionType::UNDEFINED; }
};
...
class UndefinedExpression : public Expression
{
    ...
    public:
        UndefinedExpression(Token &_var) : var(_var)
        {
...
{    
  protected:
    ...
    std::shared_ptr<Condition> stop_cond;
  ...
  public:
    ...
    ForStatement(std::unique_ptr<Statement> &_start,
                 std::unique_ptr<Condition> &_stop_cond,
    ...
    ForStatement(const ForStatement &_for)
        ...
        , stop_cond(std::move(_for.stop_cond))
    ...
    auto getStopCond() { return stop_cond.get(); }
...
{    
  protected:
    std::shared_ptr<Condition> continue_cond;
    std::vector<std::shared_ptr<Statement>> block;

    std::unordered_map<std::string, ValueType::Type> block_local_vars;

  public:
    WhileStatement(std::unique_ptr<Condition> &_continue_cond,
                 std::vector<std::shared_ptr<Statement>> &_block,
                 std::unordered_map<std::string, 
                                    ValueType::Type> &_block_local_vars)
    {
        ...
        continue_cond = std::move(_continue_cond);
        block = std::move(_block);

        block_local_vars = _block_local_vars;
    }

    WhileStatement(const WhileStatement &_for)
        : continue_cond(std::move(_for.continue_cond))
        , block(std::move(_for.block))
        , block_local_vars(_for.block_local_vars)
    {}

    auto getContinueCond() { return continue_cond.get(); }
    auto &getBlock() { return block; }
    auto getBlockVars() { return &block_local_vars; }
...
class Parser
{
  ...
  protected:
    ...
    int strictTypeCheckReturn(Token &_tok, bool is_index_or_deref = false)
    {
        if (cur_expr_type == ValueType::Type::MAX) return 0;
