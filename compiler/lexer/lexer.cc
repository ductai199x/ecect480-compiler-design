...
void Lexer::parseLine(std::string &line)
{
    ...
    for (auto iter = line.begin(); iter != line.end(); iter++) {
        ...
        if (isType<int>(cur_token_str)) {
            Token _tok(Token::TokenType::TOKEN_INT, cur_token_str, cur_line);
            toks_per_line.push(_tok);
            continue;
        } 
        else if (isType<float>(cur_token_str)) {
            Token _tok(Token::TokenType::TOKEN_FLOAT, cur_token_str, cur_line);
            toks_per_line.push(_tok);
            continue;
        }
        ...
        if (auto k_iter = keywords.find(cur_token_str);
            ...
            Token _tok(Token::TokenType::TOKEN_IDENTIFIER, literal, cur_line);
            toks_per_line.push(_tok);
        }
    }
}
